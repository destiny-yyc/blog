...
categories: [前端面试题]
author: 枫🍁川
permalink: interview-javascript.html
hide: false
---
...
同样是检测对象obj调用toString方法，`obj.toString()`的结果和`Object.prototype.toString.call(obj)`的结果**不一样**，这是为什么？
...
- `nul`l 的值是机器码 NULL 指针(null 指针的值全是 0)
...
### 8. JavaScript 类数组对象的定义？ 对类数组对象的理解，如何转化为数组
...
### 16. JavaScript为什么要进行变量提升，它导致了什么问题？
...
- **CommonJS**是对模块的浅拷⻉，ES6 Module是对模块的引⽤，即ES6 Module只存只读，不能改变其值，也就是指针指向不能变，类似const；
- **import**的接⼝是read-only（只读状态），不能修改其变量值。 即不能修改其变量的指针指向，但可以改变变量内部指针指向，可以对commonJS对重新赋值（改变指针指向），但是对ES6 Module赋值会编译报错。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c68fcad75ea54d62a9404aa02cafc65c~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
